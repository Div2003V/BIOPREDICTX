...
# Load new database for multi-symptom matching
bio_db = pd.read_csv("kaggle_dataset/biopredictx_database.csv")

# Load trained model
Rf = pickle.load(open('model/RandomForest.pkl', 'rb'))

# Symptoms list and Diseases list (FULL)
...
# Function to get old database information
    ...
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms" or symptoms.strip() == "":
            ...
            if len(corrected_symptoms) == 1:
                ...
                for idx, row in bio_db.iterrows():
                    matched = 0
                    symptoms = [row['Symptom 1'], row['Symptom 2'], row['Symptom 3']]
                    for symptom in corrected_symptoms:
                        if symptom.lower() in [s.lower() for s in symptoms if pd.notna(s)]:
                            matched += 1
                    if matched >= len(corrected_symptoms):
                        possible_diseases.append(row)

                if possible_diseases:
                    row = possible_diseases[0]
                    predicted_disease = row['Disease Name']
                    dis_des = row['Description']
                    my_precautions = eval(row['Precautions'])
                    medications = eval(row['Medications'])
                    rec_diet = eval(row['Diet'])

                    return render_template('index.html', symptoms=corrected_symptoms, predicted_disease=predicted_disease,
                                           dis_des=dis_des, my_precautions=my_precautions, medications=medications,
                                           my_diet=rec_diet)
                else:
                    message = "No matching disease found with given symptoms."
                    return render_template('index.html', message=message)
